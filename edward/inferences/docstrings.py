...
Note that args are written with newlines at end such that they're
easily added to. (For example, return values don't have an extra new
line.) You must remove the last newline.

Args below represent a global vocabulary of arguments shared across
more than one inference algorithm. They are sorted alphabetically.
"""
...
arg_discriminator = """
  discriminator: function.
    ...
    in $[0, 1]$.
"""[1:]
arg_independent_chain_ndims = """
  independent_chain_ndims: States must be vectorized.
"""[1:]
arg_grads_target_log_prob = """
...
"""[1:]
arg_state = """
  state: Tensor or list of Tensors. Each element is a
